/**
 * @cz
 * this file is modified from skynet_server.c in skynet
 * modify logic for droping foreign message
*/
#include "skynet.h"
...
static inline void foreign_message_drop(struct skynet_message* msg) {
	if((msg->sz >> MESSAGE_TYPE_SHIFT) == PTYPE_FOREIGN) {
		...
		skynet_foreign_dropseri(msg->data, sz);
	}
}
...
drop_message(struct skynet_message *msg, void *ud) {
	struct drop_t *d = ud;
	if((msg->sz >> MESSAGE_TYPE_SHIFT) == PTYPE_FOREIGN) { // @cz drop for foreign serialize
		size_t sz = msg->sz & MESSAGE_TYPE_MASK;
		skynet_foreign_dropseri(msg->data, sz);
	}
...
skynet_send(struct skynet_context * context, uint32_t source, uint32_t destination , int type, int session, void * data, size_t sz) {
	if ((sz & MESSAGE_TYPE_MASK) != sz) {
		...
		if (type & 0xff == PTYPE_FOREIGN) { // @cz drop for foreigin serialize
			skynet_foreign_dropseri(data, sz);
		}
